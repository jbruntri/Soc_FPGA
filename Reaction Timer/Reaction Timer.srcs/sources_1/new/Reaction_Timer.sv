...
module Reaction_Timer(
    ...
    logic [3:0] d3_tmp, d2_tmp, d1_tmp, d0_tmp; // Temporary Display Value Registers
    logic bcd_ready;                            // BCD Conversion Data Ready
    logic bin_conv;                             // begin conversion
    // Random Number Placeholder
    
    //Create Display Item;
    disp_hex_mux display (.clk(clk), .reset(1'b0), .val3(d3), .val2(d2),
                          .val1(d1), .val0(d0), .dp_in(4'b1111), .an(an), .sseg(sseg));
    
    // Create Binary to BCD Converter
    //bin_to_bcd bcdconv (.clk(clk), .reset(1'b0), .start(bin_conv), .bin(refl_reg),
    //                    .ready(bcd_ready), .done_tick(), .val3(d3_tmp), .val2(d2_tmp), .val1(d1_tmp), .val0 (d0_tmp)); 
        ...
        begin
            ...
            case (state_reg)
                ready:                              // Initialization State/Waiting to Start
                    begin
                        random_reg = $urandom_range(2000, 15000);
                        rgb_led2 = 3'b001;          // Set RGB LED to Blue for Ready/Start States
                        stim = 1'b0;              // Turn off Stimulus LED 
                        bin_conv =1'b0;             // disable BCD conversion
                        d3_tmp = 4'b0000;
                        d2_tmp = 4'b0000;
                        d1_tmp = 4'b0000;
                        d0_tmp = 4'b0000;
                        ms_next = 0;                // clear ms count
                    ...
                    begin
                        ...
                        if(btnl)                    // Hit stop button before game began
                            ...
                            begin
                                ...
                                stim = 1'b1;          // Turn on Stimulus LED
                            end
                ...
                win:                                // Win State
                    begin
                        ...
                        bin_conv = 1'b1;            // convert time to BCD                              
                    end
                lose:
                    begin
